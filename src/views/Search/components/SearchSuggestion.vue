<template>
  <div>
    <van-cell
      v-for="(item, index) in hightLightSuggestionsList"
      :key="index"
      icon="search"
      @click="$emit('change_keywords', suggestionsList[index])"
    >
      <template #title>
...
export default {
  ...
  methods: {
    getSuggestions: debounce(async function () {
      try {
        ...
        this.suggestionsList = data.data.options.filter(Boolean)
      } catch (error) {
        if (error.response) {
  ...
  computed: {
    hightLightSuggestionsList() {
      const reg = new RegExp(this.keywords, 'ig')
      return this.suggestionsList.map((str) => {
